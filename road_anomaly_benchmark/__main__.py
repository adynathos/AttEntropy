...
$(document).ready(() => {
	...
	document.querySelector('table').querySelectorAll('tr :first-child').forEach((name_el) => {
		if(name_el.textContent.includes('SETR')) {
			name_el.parentElement.style.backgroundColor = 'rgb(200, 255, 200)'
		}

		if(name_el.textContent.includes('Segformer')) {
			name_el.parentElement.style.backgroundColor = 'rgb(200, 200, 255)'
		}

		if(name_el.textContent.includes('LOST')) {
			name_el.parentElement.style.backgroundColor = 'rgb(250, 200, 200)'
		}
	})
})
"""
